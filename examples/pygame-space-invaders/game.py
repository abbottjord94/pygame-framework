...
class Game():
	...
	def run(self):
		...
		self._gameManager = self.instantiate(GameManager(self,"GameManager","unknown.png",-999,-999))
		...
		while running:
			...
			#Handle keyboard and mouse events here (maybe pass this to the GameManager?)
			...
			for g in self._gameObjects:
				...
				for k in self._gameObjects:
					if not k==g and g._bounds.colliderect(k._bounds):
						g.collision(k)
